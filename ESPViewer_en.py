...
def render_current_view_tachyon(port, vmd_dir, tachyon_exe, output_path,
    ...
    log(f"  Rendering current view: port={port}, vmd_dir={vmd_dir}")
    ...
    log(f"  Running Tachyon: {tachyon_exe}")
    try:
        result = subprocess.run(args, capture_output=True, cwd=vmd_dir, timeout=600, text=True)
        if result.returncode != 0:
            log(f"  ✗ Tachyon failed (exit code: {result.returncode})")
            if result.stderr:
                log(f"    Error info: {result.stderr[:500]}")
            return None
        log(f"  ✓ Tachyon render complete")
    except subprocess.TimeoutExpired:
        log("  ✗ Tachyon timeout")
        return None
    except FileNotFoundError:
        log(f"  ✗ Tachyon executable not found: {tachyon_exe}")
        return None

    # Check output
    bmp_path = os.path.join(vmd_dir, bmp_name)
    if not os.path.exists(bmp_path):
        log(f"  ✗ Tachyon output file not found: {bmp_path}")
        return None
    log(f"  ✓ BMP file generated: {os.path.getsize(bmp_path)} bytes")
...
class JobWorker(QThread):
    ...
    def _run_pick(self):
        ...
        if not vmd_port:
            self.error_signal.emit("VMD not started")
            return
        resp = send_vmd_cmd(vmd_port, "mouse mode 4", timeout=3)
        if "ERROR" in resp:
            self.log(f"⚠ VMD pick mode failed: {resp}")
        else:
            self.log("")
            self.log("━━━ Query Extrema ━━━")
            self.log("VMD pick mode active. Click extrema spheres to query ESP")
            self.log("(C = Maxima, O = Minima)")
            self.set_status("Pick mode: Click extrema spheres in VMD (C/O atoms)")
    ...
    def _run_render_view(self):
        ...
        self.log(f"\nRendering current view ({resolution[0]}x{resolution[1]})...")
        t0 = time.time()
        ...
        dt = time.time() - t0
        if result:
            self.log(f"✓ Render complete ({dt:.1f}s): {result}")
            self.finished_signal.emit("render_view", result)
        else:
            self.log(f"✗ Render failed ({dt:.1f}s)")
            self.error_signal.emit("Tachyon render failed, check log")
    ...
    def _run_pipeline(self, multiwfn_exe, vmd_exe, vmd_dir, mode,
        ...
        self.set_status("Starting calculation...")
        self.set_progress(0)

        for idx, fch_path in enumerate(input_files):
            sys_num = idx + 1
            self.log(f"\n{'='*50}")
            self.log(f"Processing file {sys_num}/{nsystems}: {os.path.basename(fch_path)}")
            ...
            def make_progress_cb(file_idx, total_files, completed_val, total_val):
                def cb(line):
                    ...
                    if len(msg) > 80:
                        msg = msg[:77] + "..."
                    self.set_status(f"[File {file_idx}/{total_files}] {msg}")
            ...
            if mode in ("pt", "all"):
                ...
                self.set_status(f"Calculating {step_label}: {os.path.basename(fch_path)}")
        ...
        self.log(f"\n{'='*50}")
        self.log("Generating VMD script...")
        self.set_status("Generating VMD script...")
        ...
        if mode == "pt":
            ...
            script = generate_vmd_script_all(multiwfn_exe, vmd_dir, nsystems,
                                            color_low, color_high)
            self.log("ALL Mode: Combined script generated (ISO isosurface + EXT extrema)")

        self.log(f"VMD script: {script}")
        ...
        self.log(f"Launching VMD preview (interactive, port {vmd_port}, callback port {callback_port})...")
        self.set_status("Launching VMD...")
        ...
        for pattern in patterns:
            for f in glob.glob(os.path.join(vmd_dir, pattern)):
                try:
                    ...
                    pass
        if cleaned > 0:
            self.log(f"Cleaned {cleaned}  old file(s)")
            ...
            with open(pdb_path, "r", encoding="utf-8", errors="replace") as f:
                for line in f:
                    if line.startswith("HEADER") or line.startswith("TITLE") or line.startswith("REMARK"):
                        if "eV" in line:
                            self.log(f"  Unit detection: eV (from {os.path.basename(pdb_path)})")
                            return
                    if line.startswith("HETATM") or line.startswith("ATOM"):
                        break
            self.log("  Unit detection: kcal/mol (default)")
        except Exception:
            pass
    ...
    def _run_area_chart(self):
        ...
        self.set_busy(True)
        self.set_status("Calculating ESP area distribution...")
        self.log("")
        self.log("━━━ ESP Area Distribution Analysis ━━━")
        self.log(f"Range: {range_low} ~ {range_high} kcal/mol, Num bins: {n_bins}")
        ...
        for idx, fch_path in enumerate(input_files):
            sys_num = idx + 1
            fch_name = os.path.basename(fch_path)
            self.log(f"\nAnalyzing file {sys_num}/{n_total}: {fch_name}")
            ...
            def area_cb(line):
                ...
                if len(stripped) > 3:
                    ...
                    self.set_status(f"[Area analysis {sys_num}/{n_total}] {stripped}")
            ...
            if data:
                ...
                self.log(f"  Obtained {n_found}  bins, total surface area = {total_area:.2f} \u00c5\u00b2")
            else:
                if not ok:
                    self.log("  Multiwfn error and no area data parsed")
                else:
                    self.log("  Multiwfn finished OK but no area data parsed")

        if not all_data:
            self.log("\n✗ No valid area distribution data")
            self.set_busy(False)
        ...
        for fname, data in all_data:
            ...
            self.log(f"\nGenerating histogram: {os.path.basename(chart_path)}")
            ...
            if result_path:
                self.log(f"✓ Chart saved: {result_path}")
                results.append(result_path)
            else:
                self.log("✗ Chart generation failed")

            data_path = os.path.join(out_dir, f"{base}_esp_area_data.txt")
            saved = save_area_data_file(data, data_path)
            if saved:
                self.log(f"✓ Data file saved: {saved}")
                results.append(saved)

        self.set_progress(100)
        self.set_status("✓ ESP area chart complete")
        self.set_busy(False)
...
class ESPGuiQt(QMainWindow):
    ...
    def __init__(self):
        ...
        self._apply_style()
        self.log("ESP Surface Visualization GUI v1.0 (PyQt5) started")
    ...
    def _cleanup_work_dir(self):
        if os.path.exists(self.work_dir):
            try:
                ...
                self.log(f"Cleaned temp directory: {self.work_dir}")
            except Exception as e:
                self.log(f"Failed to clean temp dir: {e}")
...
QPlainTextEdit {
    ...
    def _build_ui(self):
        ...
        sub_lbl = QLabel("Multiwfn + VMD + Tachyon  |  v1.0 PyQt5 Edition")
        ...
        # ── Input Files ──
        in_grp = QGroupBox("Input Files (.fch / .fchk)")
        ...
        btn_adddir = QPushButton("Add Directory")
        ...
        self.lbl_filecount = QLabel("(0 files)")
        ...
        # ── Mode & Settings ──
        set_grp = QGroupBox("Analysis Mode & Settings")
        set_layout = QGridLayout(set_grp)

        set_layout.addWidget(QLabel("Mode:"), 0, 0)
        ...
        set_layout.addWidget(QLabel("Opacity:"), 3, 0)
        ...
        set_layout.addWidget(QLabel("Output Path:"), 4, 0)
        ...
        tab_widget.addTab(tab1, "Input & Settings")
        ...
        path_grp = QGroupBox("Path Settings")
        ...
        path_layout.addWidget(QLabel("VMD Working Directory:"), 2, 0)
        ...
        tab_widget.addTab(tab2, "Path Settings")
        ...
        self.btn_preview = QPushButton("▶ VMD Preview")
        ...
        self.btn_render_view = QPushButton("📷 Render Current View")
        ...
        self.btn_pick = QPushButton("🔍 Query Extrema")
        ...
        self.btn_area_chart = QPushButton("📊 ESP Area Chart")
        ...
        pick_grp = QGroupBox("Extrema Query Results")
        pick_layout = QVBoxLayout(pick_grp)
        pick_layout.setContentsMargins(4, 4, 4, 4)

        hdr_row = QHBoxLayout()
        self.lbl_pick_count = QLabel("(0 extrema)")
        ...
        self.pick_table.setHorizontalHeaderLabels(["#", "Type", "ESP Value"])
    ...
    def _start_callback_server(self):
        ...
        def server_loop():
            try:
                ...
                self.log_signal.emit(f"[Callback] Server started, listening on port {self.callback_port}")
                    ...
                    try:
                        ...
                        if data:
                            ...
                            if msg.startswith("PICK:"):
                                self.log_signal.emit(f"[Callback] Received VMD pick data: {msg}")
                                self.pick_signal.emit(msg)
                            else:
                                self.log_signal.emit(f"[Callback] Received unknown data: {msg[:80]}")
                    except socket.timeout:
                        continue
                    except OSError:
                        break
                    except Exception as e:
                        self.log_signal.emit(f"[Callback] Error: {e}")
                        break
                self.log_signal.emit(f"[Callback] Server stopped (port {self.callback_port})")
            except OSError as e:
                self.log_signal.emit(f"[Callback] Startup failed: {e}")
    ...
    def _handle_pick(self, msg):
        try:
            ...
            if beta_val is not None:
                ...
                self.log(
                    f"  📌 Extrema #{pdb_line} (index {atom_idx}): "
                    f"ESP = {esp_str} ({ext_type})"
                )
                self.log(f"     Coordinates: ({x}, {y}, {z})")
                self.set_status(
                    f"ESP = {esp_str} ({ext_type}, index {atom_idx})"
                )
            else:
                esp_str = "N/A"
                self.log(f"  📌 Extrema #{pdb_line} (index {atom_idx}): {ext_type}")
                self.log(f"     Coordinates: ({x}, {y}, {z})")
            ...
            for col, val in enumerate(items):
                ...
                if col == 1 and ext_type == "Maximum":
                    item.setForeground(QColor("#E65100"))
                elif col == 1 and ext_type == "Minimum":
                    item.setForeground(QColor("#0D47A1"))
                self.pick_table.setItem(row, col, item)

            self.lbl_pick_count.setText(f"({self.pick_table.rowCount()} extrema)")

        except (IndexError, ValueError) as e:
            self.log(f"  ⚠ Failed to parse VMD pick data: {msg} ({e})")

    def _clear_pick_table(self):
        self.pick_table.setRowCount(0)
        self.lbl_pick_count.setText("(0 extrema)")
        self.log("Extrema query table cleared")
    ...
    def _set_pick_mode_ui(self, active):
        if active:
            ...
            self.btn_pick.setText("\U0001f50d Query Extrema")
            self.btn_pick.setStyleSheet(
                "background-color: #7E57C2; border: 1px solid #5E35B1; color: white; font-weight: bold; "
                "font-size: 11pt; padding: 6px 22px; border-radius: 5px;"
            )
    ...
    def _browse_mwfn(self):
        p, _ = QFileDialog.getOpenFileName(self, "Select Multiwfn.exe", "", "EXE (*.exe)")
        if p:
            self.edit_mwfn.setText(p)

    def _browse_vmd(self):
        p, _ = QFileDialog.getOpenFileName(self, "Select vmd.exe", "", "EXE (*.exe)")
        if p:
    ...
    def _browse_vmdir(self):
        p = QFileDialog.getExistingDirectory(self, "Select VMD Working Directory")
        if p:
            self.edit_vmdir.setText(p)

    def _browse_tachyon(self):
        p, _ = QFileDialog.getOpenFileName(
            self, "Select tachyon_WIN32.exe", "", "EXE (*.exe)"
        )
        if p:
    ...
    def _browse_output(self):
        p, _ = QFileDialog.getSaveFileName(
            self, "Save Render Output", "", "PNG (*.png);;BMP (*.bmp)"
        )
        if p:
    ...
    def _save_paths(self):
        ...
        self.log("Path configuration saved")
    ...
    def _add_files(self):
        ...
        self.lbl_filecount.setText(f"({len(self.input_files)}  file(s))")
    ...
    def _clear_files(self):
        ...
        self.lbl_filecount.setText("(0 files)")
    ...
    def _action_render_view(self):
        if not self.vmd_port:
            QMessageBox.warning(self, "Info", "Please click [▶ VMD Preview] to launch VMD first")
            return

        tachyon_exe = self.edit_tachyon.text().strip()
        if not os.path.isfile(tachyon_exe):
            QMessageBox.critical(self, "Error", f"Tachyon not found:\n{tachyon_exe}")
            return
    ...
    def _action_pick(self):
        ...
        self.log("")
        self.log("━━━ Query Extrema ━━━")
        self.log(f"VMD port: {self.vmd_port}, Callback port: {self.callback_port}")

        if not self.callback_server:
            self.log("✗ Callback server not running! Re-launch VMD preview")
            self.set_status("Pick failed: Callback server not running")
            QMessageBox.warning(self, "Info",
                "Callback server not running, cannot receive VMD click events.\n\n"
                "Click [▶ VMD Preview] to launch VMD and try again.")
            return

        self.log("Sending pick mode command to VMD...")
        ...
        def pick_worker():
            ...
            if "ERROR" in resp:
                self.log_signal.emit(f"✗ VMD pick mode failed: {resp}")
                self.log_signal.emit("Hint: Ensure VMD window is still running and preview loaded")
                self.status_signal.emit("Pick mode failed")
                self.pick_mode_signal.emit(False)
            else:
                self.log_signal.emit("✓ VMD pick mode active (mouse mode 4)")
                self.log_signal.emit("Click extrema spheres in VMD to query ESP values")
                self.log_signal.emit("（Yellow = ESP maxima, Cyan = ESP minima）")
                self.log_signal.emit("Hint: Click VMD graphics window to activate, then click spheres")
                self.status_signal.emit("Pick mode: Click extrema spheres in VMD (C/O atoms)")
    ...
    def _action_area_chart(self):
        if not self.input_files:
            QMessageBox.critical(self, "Error", "Please add input files first")
            return

        multiwfn_exe = self.edit_mwfn.text()
        if not os.path.isfile(multiwfn_exe):
            QMessageBox.critical(self, "Error", f"Multiwfn not found:\n{multiwfn_exe}")
            return
        ...
        if not _HAS_MPL:
            QMessageBox.critical(
                ...
                "Please run: pip install matplotlib numpy"
            )
            return
        ...
        if n_bins < 2 or n_bins > 50:
            QMessageBox.critical(self, "Error", "Number of bins must be between 2 and 50")
            return
    ...
    def _start_job(self, action):
        ...
        if not os.path.isfile(vmd_exe):
            QMessageBox.critical(self, "Error", f"VMD not found:\n{vmd_exe}")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", "Color range or point size is not a valid number")
            return
